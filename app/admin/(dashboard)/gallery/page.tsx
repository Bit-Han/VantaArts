...
useEffect(() => {
  fetch("/api/admin/projects")
    .then((r) => r.json())
    .then((data: { id: string; title: string; category: string }[] | { error: string }) =>
  ...
  useEffect(() => {
    fetch("/api/admin/gallery")
			.then((r) => r.json())
			.then((data: GalleryItemRow[] | {error: string}) => setItems(Array.isArray(data) ? data : []));
  }, []);
  ...
  const handleUpload = async (files: FileList | null) => {
    ...
    const data: GalleryItemRow[] | { error: string } = await res.json();
...
const updateProject = async (id: string, project_id: string | null) => {
	setItems((prev) => prev.map((i) => (i.id === id ? { ...i, project_id } : i)));
	await fetch("/api/admin/gallery", {
		method: "PUT",
		headers: { "Content-Type": "application/json" },
          ...
          >
            ...
            <div className="p-3 flex flex-col gap-2">
              ...
              <select
                ...
                onChange={(e) => updateProject(item.id, e.target.value || null)}
              >
